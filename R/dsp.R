...
dsp <- function(x,y, p.pch=19, p.col=1, p.cex=.8, bkgr=T, ...){
    ...
    cc <- complete.cases(x,y)
        if(length(p.pch) == 1){ p.pch <- rep(p.pch,L) }
        if(length(p.col) == 1){ p.col <- rep(p.col,L) }
        if(length(p.cex) == 1){ p.cex <- rep(p.cex,L) }
    x <- x[cc] ; y <- y[cc] ; p.pch <- p.pch[cc] ; p.col <- p.col[cc] ; p.cex <- p.cex[cc]
        
        x.levels <- sort(unique(x)) ;  y.levels <- sort(unique(y))
    tab <- table(x,y)
    max.freq <- max(tab)
    box.size <- ceiling( sqrt(max.freq) )
        X <- range(x)+c(0,1) ; Y <- range(y)+c(0,1)
    ...
    coord <- (1:box.size) / (box.size+1) ; off.set <- coord[1]/4

    grey.scale <- rev(seq(.65, .95, length=max.freq))


    dat <- data.frame(id=1:length(x), x,y) ; dat <- dat[order(dat$x, dat$y),]
    within <- c(t(table(x,y))) ; within <- within[within > 0]
    idx <- NULL ; for(i in within){ idx <- c(idx, 1:i) }
    dat$idx <- idx ; dat <- dat[order(dat$id),]
    dat$col <- p.col ; dat$pch <- p.pch
    ...
    points(dat$x+coord[sc[dat$idx,1]], dat$y+coord[sc[dat$idx,2]], pch=dat$pch, col=dat$col, cex=p.cex)
